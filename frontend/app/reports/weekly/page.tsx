...
export default function WeeklyReportPage() {
  ...
  if (!report) return <section className="dashboard-panel"><p className="eyebrow">WEEKLY ROUTE REVIEW</p><h2>Mapping your seven-day progress…</h2></section>;
  ...
  return <>
    <section className="executive-hero">
      <div><p className="eyebrow">WEEKLY ROUTE REVIEW</p><h1>Your career progress, waypoint by waypoint.</h1><p className="muted">{new Date(report.period_start).toLocaleDateString()} through {new Date(report.period_end).toLocaleDateString()} · based only on recorded CareerNavIQ activity.</p></div>
    ...
    <section className="executive-grid">
      ...
      <article className="priority-card"><p className="eyebrow">NEXT-WEEK WAYPOINTS</p><h2>Recommended direction</h2><ol>{report.recommendations.map((item) => <li key={item}>{item}</li>)}</ol><div className="row wrap"><Link className="button" href="/dashboard">Open navigation hub</Link><Link className="button secondary" href="/notifications">Review route updates</Link></div></article>
    </section>

    <section className="dashboard-panel"><div className="row between"><div><p className="eyebrow">ROUTE SNAPSHOT</p><h2>Current applications by waypoint</h2></div><Link href="/applications">Open tracker →</Link></div><div className="pipeline-list">{stages.map((stage) => { const count = report.stage_counts[stage] || 0; return <div className="pipeline-row" key={stage}><span>{stage[0].toUpperCase() + stage.slice(1)}</span><div className="pipeline-track"><div className="pipeline-fill" style={{ width: `${Math.max(count ? 8 : 0, count / maxStage * 100)}%` }} /></div><strong>{count}</strong></div>; })}</div></section>

    <section className="dashboard-panel"><p className="eyebrow">NEXT WAYPOINTS</p><h2>Moves still requiring attention</h2><div className="quick-actions-grid"><Link className="quick-action" href="/notifications"><strong>{report.unread_notifications} unread updates</strong><span>Review reminders and route changes</span></Link><Link className="quick-action" href="/applications"><strong>{report.follow_ups_overdue} overdue follow-ups</strong><span>Move active opportunities forward</span></Link><Link className="quick-action" href="/interviews"><strong>{report.interviews_upcoming} upcoming interviews</strong><span>Confirm preparation and logistics</span></Link><Link className="quick-action" href="/automation"><strong>{report.matches_created} signals added</strong><span>Review and manage scheduled routes</span></Link></div></section>
  </>;
}
