...
export default function ResumesPage() {
  ...
  async function loadProfiles() {
    ...
    try {
      ...
      if (loadedProfiles.length > 0) {
        setProfileId((current) => current || String(loadedProfiles[0].id));
      }
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Unable to load career profiles."
      );
    } finally {
  ...
  async function loadResumes(id: string) {
    ...
    try {
      ...
      setSelectedId((current) => {
        if (current && loadedResumes.some((resume) => resume.id === current)) {
          return current;
        }

        return loadedResumes[0]?.id ?? null;
      });
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Unable to load résumés."
      );
    } finally {
  ...
  async function handleUpload() {
    ...
    if (!profileId) {
      setError("Select a career profile first.");
      return;
    }

    if (!uploadFile) {
      setError("Choose a PDF, DOCX, or text résumé file.");
      return;
    }

    if (!resumeName.trim()) {
      setError("Enter a résumé name.");
      return;
    }
  ...
  async function handleAnalyze(resume: Resume) {
    ...
    try {
      const updated = await api(`/api/resumes/${resume.id}/analyze`, {
        method: "POST",
      });
  ...
  async function handlePrimary(resume: Resume) {
    ...
    try {
      const updated = await api(`/api/resumes/${resume.id}/primary`, {
        method: "POST",
      });
      ...
      setMessage(`${updated.name} is now the primary résumé.`);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Unable to set primary résumé."
      );
    } finally {
      setWorkingId(null);
    }
  ...
  async function handleDelete(resume: Resume) {
    ...
    try {
      await api(`/api/resumes/${resume.id}`, {
        method: "DELETE",
      });
  ...
  return (
    <>
      ...
      {message && (
        <section className="resume-alert resume-alert-success">
          <strong>Success</strong>
          <span>{message}</span>
        </section>
      )}

      <section className="resume-studio-grid">
        <aside className="resume-upload-panel">
          <p className="eyebrow">ADD RÉSUMÉ</p>
          ...
          <p className="muted resume-upload-note">
        ...
        <section className="resume-library-panel">
          <div className="row between">
            ...
            <button
              type="button"
              className="secondary"
              disabled={!profileId || loadingResumes}
              onClick={() => void loadResumes(profileId)}
            >
          ...
          {loadingResumes ? (
            <div className="resume-empty-state">
              <h3>Loading résumés…</h3>
            </div>
          ) : resumes.length === 0 ? (
            <div className="resume-empty-state">
              <h3>No résumés uploaded yet</h3>
              <p className="muted">
                Upload your master résumé to begin analysis and tailoring.
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="row between">
                    <div>
                      ...
                      <p className="muted">{resume.original_filename}</p>
                    </div>

                    <div className="resume-score-badge">
                      <strong>{resume.analysis_score ?? "—"}</strong>
                      <small>score</small>
                    </div>
                  </div>
                  ...
                  <p className="resume-preview">
                    {resume.extracted_text_preview ||
                      "No readable preview is available."}
                  </p>
          ...
          {!selectedResume ? (
            ...
            <>
              ...
              <div className="resume-analysis-section">
                <h3>Summary</h3>
                <p>
                  {selectedResume.analysis_summary ||
                    "Run analysis to generate a résumé summary."}
                </p>
              </div>
              ...
              <div className="resume-analysis-section">
                ...
                {selectedResume.gaps.length ? (
                  <ul>
                    {selectedResume.gaps.map((gap, index) => (
              ...
              <div className="resume-analysis-section">
                <h3>Metrics detected</h3>

                {selectedResume.metrics_found.length ? (
                  <div className="resume-metric-tags">
                    {selectedResume.metrics_found.map((metric, index) => (
                      <span className="badge" key={`${metric}-${index}`}>
                        {metric}
                      </span>
                    ))}
                  </div>
                  ...
                  <p className="muted">
                    No quantified achievements detected yet.
                  </p>
                )}
              </div>
              ...
              <button
                type="button"
                disabled={workingId === selectedResume.id}
                onClick={() => void handleAnalyze(selectedResume)}
              >
                {workingId === selectedResume.id
                  ? "Analyzing…"
                  : "Run résumé analysis"}
              </button>
            </>
