...
export default function CalendarPage() {
  ...
  if (loading) return <section className="dashboard-panel"><p className="eyebrow">CAREER TIMELINE</p><h2>Loading your calendar…</h2></section>;
  ...
  return <>
    ...
    <section className="executive-grid">
      <article className="priority-card"><p className="eyebrow">TODAY'S AGENDA</p><h2>Work in priority order</h2><div className="activity-list">{data.agenda.map((item, index) => <div className="activity-item" key={`${item.title}-${index}`}><span className="badge">{item.priority}</span><strong>{item.title}</strong><small>{item.detail}</small><Link href={item.link}>Open task →</Link></div>)}</div></article>
    ...
    <section className="dashboard-panel">
      <div className="row between"><div><p className="eyebrow">CAREER TIMELINE</p><h2>Scheduled activity</h2></div><Link href="/notifications">Open alerts →</Link></div>
