...
export default function ApplicationDetail(){
 ...
 async function exportPackage(){setBusy(true);setError("");try{const result=await api(`/api/enterprise/applications/${id}/export`,{method:"POST"});alert(`Application package manifest created. Export ID: ${result.id}`)}catch(e){setError(e instanceof Error?e.message:"Export failed")}finally{setBusy(false)}}
 async function copy(text:string){await navigator.clipboard.writeText(text)}

 if(!d)return <p>Loading…</p>;
 const p=d.interview_prep;

 return <><div className="hero"><p className="eyebrow">APPLICATION ROUTE</p><h1>{d.job.title}</h1><p className="muted">{d.job.company} · {d.job.location}</p><div className="row wrap"><a className="button secondary" href={d.job.url} target="_blank">Posting</a><button onClick={prep} disabled={busy}>{p?"Regenerate interview prep":"Generate interview prep"}</button><button onClick={generatePackage} disabled={busy}>{pkg?"Regenerate full package":"Generate full package"}</button><button className="secondary" onClick={generateSalaryPlan} disabled={busy}>Generate salary plan</button><button className="secondary" onClick={exportPackage} disabled={busy}>Create application package manifest</button></div></div>
 {error&&<p className="error">{error}</p>}
 <div className="two-col"><section className="card"><h2>Route details</h2><label>Status</label><select value={d.status} onChange={e=>update({status:e.target.value})}>{["wishlist","applied","recruiter","interview","final","offer","accepted","rejected"].map(x=><option key={x}>{x}</option>)}</select><label>Recruiter name</label><input defaultValue={d.recruiter_name} onBlur={e=>update({recruiter_name:e.target.value})}/><label>Recruiter email</label><input defaultValue={d.recruiter_email} onBlur={e=>update({recruiter_email:e.target.value})}/><label>Next action</label><input defaultValue={d.next_action} onBlur={e=>update({next_action:e.target.value})}/><label>Notes</label><textarea rows={8} defaultValue={d.notes} onBlur={e=>update({notes:e.target.value})}/></section><section className="card"><h2>Application assets</h2><p><strong>Alignment:</strong> {d.match_score??"Not scored"}%</p>{d.tailoring?<><p><strong>Résumé:</strong> {d.tailoring.version_name}</p><p><strong>ATS score:</strong> {d.tailoring.ats_score}</p></>:<p className="muted">No tailored résumé attached.</p>}</section></div>

 {matchExplanation&&<section className="card"><div className="row between"><div><p className="eyebrow">ALIGNMENT EXPLANATION</p><h2>{matchExplanation.recommendation}</h2></div><div className="job-score">{matchExplanation.overall}<small>alignment</small></div></div><div className="score-bars"><span>Title <b>{matchExplanation.details.title}</b></span><span>Keywords <b>{matchExplanation.details.keywords}</b></span><span>Location <b>{matchExplanation.details.location}</b></span><span>Résumé <b>{matchExplanation.details.resume}</b></span></div><div className="two-col"><div><h3>Strengths</h3>{matchExplanation.strengths.map((x:string,i:number)=><p key={i}>✓ {x}</p>)}</div><div><h3>Gaps</h3>{matchExplanation.gaps.map((x:string,i:number)=><p className="warn-text" key={i}>• {x}</p>)}</div></div></section>}
