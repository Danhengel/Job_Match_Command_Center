  ...
  return <>
    ...
    <div className="interview-layout">
      ...
      <section className="studio-panel interview-priority">
        ...
        {nextEvent ? <><span className="badge">{nextEvent.event_type}</span><h2>{nextEvent.title}</h2><p className="muted">{new Date(nextEvent.starts_at).toLocaleString()}</p><p>{nextEvent.notes || "Open the application workspace to review the role, match evidence, tailored résumé, and talking points."}</p><div className="row wrap">{nextEvent.meeting_url ? <a className="button" href={nextEvent.meeting_url} target="_blank" rel="noreferrer">Join meeting</a> : null}<Link className="button secondary" href={`/applications/${nextEvent.application_id}`}>Prepare application</Link></div></> : <><h2>No upcoming interviews</h2><p className="muted">Add a recruiter call or interview when an application advances.</p></>}
        <div className="interview-checklist"><strong>Preparation checklist</strong><span>Review the job description and match gaps</span><span>Select three quantified STAR stories</span><span>Prepare five questions for the interviewer</span><span>Confirm meeting details and follow-up plan</span></div>
      </section>
    </div>
