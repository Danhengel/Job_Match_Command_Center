...
export default function InterviewCenter() {
  ...
  return <>
    <section className="executive-hero interview-hero">
      <div><p className="eyebrow">INTERVIEW PATH</p><h1>Navigate every conversation with confidence.</h1><p className="muted">Keep every interview, recruiter call, preparation task, and follow-up connected to the right opportunity route.</p></div>
      <div className="executive-actions"><Link className="button secondary" href="/applications">Application tracker</Link><Link className="button" href="/crm">Network map</Link></div>
    </section>
    ...
    <div className="interview-layout">
      ...
      <section className="studio-panel interview-priority">
        ...
        {nextEvent ? <><span className="badge">{nextEvent.event_type}</span><h2>{nextEvent.title}</h2><p className="muted">{new Date(nextEvent.starts_at).toLocaleString()}</p><p>{nextEvent.notes || "Open the application route to review the role, alignment evidence, tailored résumé, and talking points."}</p><div className="row wrap">{nextEvent.meeting_url ? <a className="button" href={nextEvent.meeting_url} target="_blank" rel="noreferrer">Join meeting</a> : null}<Link className="button secondary" href={`/applications/${nextEvent.application_id}`}>Prepare application</Link></div></> : <><h2>No upcoming interviews</h2><p className="muted">Add a recruiter call or interview when an application advances.</p></>}
        <div className="interview-checklist"><strong>Preparation checklist</strong><span>Review the opportunity details and alignment gaps</span><span>Select three quantified STAR stories</span><span>Prepare five questions for the interviewer</span><span>Confirm meeting details and follow-up plan</span></div>
      </section>
    </div>
