...
function due(contact: Contact) {
  return Boolean(
    ...
    && new Date(contact.next_follow_up_at).getTime() <= Date.now(),
  );
}
...
export default function RecruiterCRM() {
  ...
  const filtered = useMemo(
    () => items
      ...
      .sort(
        (left, right) =>
          Number(due(right)) - Number(due(left))
  ...
  const activeCount = items.filter(
    (item) => !["closed", "inactive"].includes(item.status),
  ).length;
  const strongCount = items.filter(
    (item) => item.relationship_score >= 70,
  ).length;
  ...
  async function patch(id: number, payload: Record<string, unknown>) {
    ...
    setError("");
    try {
      await api(`/api/recruiting/recruiters/${id}`, {
        method: "PATCH",
  ...
  return (
    <>
      ...
      <section className="interview-kpis">
        <article><span>Contacts</span><strong>{items.length}</strong><small>total relationships</small></article>
        <article><span>Active</span><strong>{activeCount}</strong><small>open relationships</small></article>
        <article><span>Follow-ups due</span><strong>{dueCount}</strong><small>need attention</small></article>
        <article><span>Strong relationships</span><strong>{strongCount}</strong><small>score 70 or higher</small></article>
      </section>
      ...
      <div className="crm-layout">
        <form className="studio-panel crm-form" onSubmit={create}>
          ...
          <h2>New recruiter or hiring manager</h2>
          ...
          <div className="interview-form-grid">
            <div>
              ...
              >
                {["new", "contacted", "responded", "active", "interviewing", "closed", "inactive"].map(
          ...
          <input
            ...
            onChange={(event) => setForm({ ...form, relationship_score: Number(event.target.value) })}
          />
          <label>Notes</label>
          <textarea
            rows={5}
            value={form.notes}
            placeholder="How you met, relevant roles, last conversation, and next step"
        ...
        <section className="studio-panel crm-priority">
          ...
          <h2>Work the right follow-ups first.</h2>
          <p className="muted">
            Contacts are automatically ordered by overdue follow-up and then
            by your relationship priority score.
          </p>
          <div className="interview-checklist">
            <strong>High-value follow-up rhythm</strong>
            <span>Send a thank-you within 24 hours</span>
            <span>Follow up after 5–7 business days</span>
            <span>Share a concise, relevant update</span>
            <span>Record the next action while context is fresh</span>
          </div>
          <p className="studio-trust-note">
            Relationship scores are your own prioritization tool.
            They are not predictions of recruiter influence or hiring outcomes.
          </p>
        </section>
      </div>
      ...
      <section className="dashboard-panel">
        ...
        <div className="crm-contact-grid">
          {filtered.map((item) => (
            <article
              className={`crm-contact-card ${due(item) ? "followup-due" : ""}`}
              key={item.id}
            >
              <div className="row between">
                <div>
                  <div className="row wrap">
                    ...
                    {due(item) ? <span className="badge warning-badge">Follow-up due</span> : null}
                  </div>
                  <h3>{item.name}</h3>
                  <p className="muted">{item.title || "Contact"} · {item.company}</p>
                </div>
                <div className="crm-score">
                  <strong>{item.relationship_score}</strong>
              ...
              <div className="crm-meta">
                ...
                {item.next_follow_up_at
                  ? <span>Follow up {new Date(item.next_follow_up_at).toLocaleString()}</span>
                ...
                <input
                  ...
                  onChange={(event) => {
                    if (event.target.value) {
                      void patch(item.id, {
                        next_follow_up_at: new Date(event.target.value).toISOString(),
                      });
                    }
                  }}
                />
